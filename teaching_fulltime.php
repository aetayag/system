...
$stmt = $pdo->prepare("
    ...
    FROM file_201
    WHERE LOWER(TRIM(employee_type)) = 'full-time'
");
...
th, td { border: 1px solid #444; padding: 8px; text-align: left; white-space: nowrap; cursor: pointer; }
tbody tr:nth-child(even) { background: #e6e6e6; }
tbody tr.selected { background-color: #a0d2a0; }

.bottom-right-btn {
    position: fixed;
    bottom: 20px;
    right: 20px;
    background-color: #999; /* inactive by default */
    color: white;
    padding: 12px 24px;
    border-radius: 6px;
...
.bottom-right-btn.active {
    background-color: #1f8f2d;
    pointer-events: auto; /* enable click */
}
.bottom-right-btn.active:hover {
            ...
            <tbody>
                <?php if ($employees): ?>
                    <?php foreach ($employees as $emp): ?>
                        <tr data-id="<?= htmlspecialchars($emp['id']) ?>" data-name="<?= htmlspecialchars($emp['name']) ?>">
...
// When Set Schedule clicked
setBtn.addEventListener('click', () => {
    if(!selectedRow) return;

    const instructorId = selectedRow.dataset.id;
    const instructorName = selectedRow.dataset.name;

    // Redirect to set_schedule.php with GET
    window.location.href = `set_schedule.php?instructor_id=${instructorId}&name=${encodeURIComponent(instructorName)}`;
});
</script>
