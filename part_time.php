...
/* ===== PROFILE IMAGE (NOT FROM POSTING) ===== */
...
/* ===== FETCH ANNOUNCEMENTS (IMAGES FROM POSTING) ===== */
$stmt = $pdo->prepare("
    SELECT id, title, message,
           -- prepend folder path to image if exists
           CASE 
               WHEN image IS NOT NULL AND image != '' THEN CONCAT('../hr system/uploads/', image) 
               ELSE NULL 
           END AS image,
           created_at
    FROM posting
...
<div class="wrapper">
   ...
   <!-- REQUEST SCHEDULE (NON-EXPANDABLE) -->
<div class="sidebar-icon">
    <a href="set_schedule.php" style="display:flex; align-items:center; gap:12px; width:100%; text-decoration:none; color:inherit;">
        <i data-feather="calendar"></i>
        <span class="icon-label">Request Schedule</span>
    </a>
</div>
...
$stmt = $pdo->prepare(
    ...
    WHERE f.employee_type LIKE 'part%'
